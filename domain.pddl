(define (domain restaurant)
  ...
  (:action move
    ...
    :precondition (and (at ?r ?loc1) (adjacent ?loc1 ?loc2) (not (hasPlate ?r)))
    :effect (and (at ?r ?loc2) (not (at ?r ?loc1)))
  )

  (:action move_plate
    :parameters (?r - robot ?p - plate ?loc1 - location ?loc2 - location)
    :precondition (and (at ?r ?loc1) (adjacent ?loc1 ?loc2) (holding ?r ?p))
    :effect (and (at ?r ?loc2) (not (at ?r ?loc1)) (at ?p ?loc2) (not (at ?p ?loc1)))
  )
)
